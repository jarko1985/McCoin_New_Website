              ...
              <tbody>
                {block
                  ? Object.entries(block).map(([key, value], idx) => (
                      ...
                      >
                        <td className="p-2 font-semibold capitalize text-white/80 whitespace-nowrap">
                          {key.replace(/_/g, ' ')}
                        </td>
                        <td className="p-2 text-white/90 break-words max-w-xs">
                          {value?.toString()}
                        </td>
                      </tr>
                    ))
        ...
        <div className="overflow-x-auto">
          ...
          <table className="min-w-full text-xs sm:text-sm border border-white/10 rounded-xl overflow-hidden">
            <thead className="bg-[#EC3B3B] text-white text-left">
              <tr>
                <th className="p-2 sm:p-3">{t('transactionColumns.txId')}</th>
                <th className="p-2 sm:p-3">{t('transactionColumns.vout')}</th>
                <th className="p-2 sm:p-3">{t('transactionColumns.size')}</th>
                <th className="p-2 sm:p-3">{t('transactionColumns.weight')}</th>
                <th className="p-2 sm:p-3">{t('transactionColumns.status')}</th>
                <th className="p-2 sm:p-3">{t('transactionColumns.fee')}</th>
              </tr>
            </thead>
            <tbody>
              {loading
                ...
                : txs.map((tx, i) => (
                    <motion.tr
                      ...
                      data-aos-delay={i * 20}
                    >
                      <td className="p-2 sm:p-3 text-white/80 break-words max-w-xs">
          ...
          <div className="flex flex-wrap justify-center mt-6 gap-3 items-center text-xs sm:text-sm">
            ...
            <span>{t('pagination.pageInfo', { current: page + 1, total: totalPages || '?' })}</span>
            <button
              className="bg-[#EC3B3B] px-3 py-1.5 rounded hover:bg-[#c32e2e] transition"
