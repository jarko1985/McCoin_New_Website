...
export default function BitCoinWidget({ onLoad }: { onLoad: () => void }) {
  ...
  useEffect(() => {
    const loadWidget = () => {
      try {
        ...
        script.onerror = () => {
          setError('Failed to load TradingView BTC widget');
          onLoad();
        };
        ...
        setError('Error initializing BTC widget');
        console.error(err);
        onLoad();
      }
    };
            ...
            {!loaded && !error && (
              <div className="absolute inset-0 flex items-center justify-center bg-[#07153b] rounded-xl">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-gray-300 text-sm">Loading BTC data...</p>
                </div>
              </div>
            )}
