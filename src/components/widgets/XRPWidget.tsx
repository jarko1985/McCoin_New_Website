...
export default function RippleWidget({ onLoad }: { onLoad: () => void }) {
  ...
  useEffect(() => {
    const loadWidget = () => {
      try {
        ...
        script.onerror = () => {
          setError('Failed to load Ripple widget');
          onLoad();
        };
        ...
        setError('Error initializing XRP widget');
        console.error(err);
        onLoad();
      }
    };
            ...
            {!loaded && !error && (
              <div className="flex items-center justify-center bg-[#07153b] rounded-xl h-[100px] w-[350px]">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-gray-300 text-xs">Loading XRP data...</p>
                </div>
              </div>
            )}
