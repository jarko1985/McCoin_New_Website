...
const LangSwitcher = () => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        asChild
        className="bg-[#07153b] border-2 border-[#EC3B3B] hover:bg-[#07153b] hover:border-2 hover:scale-3d transition-transform duration-300 cursor-pointer"
      >
        <Button
          variant="outline"
          className="flex items-center gap-2 text-[#DAE6EA] hover:text-[#DAE6EA]!"
        >
      ...
      <DropdownMenuContent align="end" className="min-w-[100px]">
        {locales.map(({ code, label, icon }) => (
          <DropdownMenuItem
            className={`text-[#07153b] cursor-pointer hover:text-[#EC3B3B]! ${
              code === currentLocale ? 'bg-[#07153b] text-white hover:bg-[#07153b]!' : ''
            }`}
            key={code}
