    ...
    <div className="mx-auto xl:max-w-[70%] bg-[#DAE6EA] dark:bg-[#07153b] text-[#07153b] dark:text-[#DAE6EA] px-4 xl:px-0 py-12">
        ...
        <div className="inline-flex gap-8 px-2 py-2 items-end">
          {blocks.length === 0 &&
            Array.from({ length: 10 }).map((_, idx) => (
              <div
                key={idx}
                className="w-44 h-52  dark:bg-[#DAE6EA]/10 rounded-md animate-pulse "
              ></div>
            ))}
                ...
                <Link href={`/en/market-explorer/${block.id}`} className="group">
                  ...
                  >
                    ...
                    <div className="relative w-full h-full preserve-3d group-hover:rotate-y-15 group-hover:-translate-y-2 transition-all duration-300">
                      ...
                      <div
                        className="absolute w-full h-full bg-[#DAE6EA] dark:bg-[#1a2d5a] dark:border-2 dark:border-[#3a4d7a] 
                      rounded-sm shadow-xl flex flex-col justify-between p-4 backface-hidden transform-style-preserve-3d dark:border-none border border-slate-400"
                      >
      ...
      <div className="text-center mt-8">
        <button
          ...
          className="bg-gradient-to-r from-[#EC3B3B] to-[#c92e2e] text-white cursor-pointer px-6 py-3 rounded-md hover:brightness-110 transition-all shadow-lg"
          disabled={loading}
        >
          {loading ? (
