...
export default function FintechNews() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await fetch(`/${locale}/api/newsdata?q=fintech`);
  ...
  if (loading) {
    return <Skeleton className="xl:max-w-[70%] px-4 xl:px-0 mx-auto h-[600px]" />;
  }
    ...
    <section className="mx-auto py-12 xl:max-w-[70%] px-4 xl:px-0">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.map((article, index) => (
          ...
          >
            <Link
              href={`/${locale}/top-news/${article.article_id}?source=newsdata&q=fintech`}
              className="block"
            >
              <img
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-[#FFF] mb-3 line-clamp-3">{article.description}</p>
