  ...
  return (
    <div className="relative w-full">
      ...
      <div className="relative h-96 w-full bg-gray-600">
        <Image src="/images/bitcoin_bg.avif" alt="News Hero" fill className="object-cover" />
        ...
        <div className="relative -mt-16 grid grid-cols-1 md:grid-cols-3 gap-0">
          {topNews.map((item, idx) => (
            <Link key={item.id} href={`/${locale}/articles/${item.id}`} passHref>
              <div
                key={item.id || idx}
                className="dark:bg-[#07153b] bg-[#DAE6EA]  shadow-xl rounded-lg border
             border-slate-600 hover:-translate-y-2 transition-transform duration-400 group cursor-pointer"
              >
                ...
                <div className="p-6 space-y-3">
                  <h2 className="text-lg text-[#07153b] dark:text-[#FFF] font-semibold line-clamp-2">
                    {item.title}
                  </h2>
                  <p className="text-sm text-[#07153b] dark:text-[#DAE6EA] line-clamp-3">
                    {item.content}
                  </p>
                  <div className="text-xs text-[#07153b] dark:text-[#DAE6EA] mt-2">
